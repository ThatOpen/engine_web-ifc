...
namespace webifc::parsing {
    ...
    {
        public:
            P21Decoder(std::vector<char> &str)
            {
                iter=str.begin();
                end=str.end();
                char c = getNext();
                for (char c=getNext(); c != 0; c=getNext())
                {
                    switch (c) {
                        case '\'':
                        {
                            ...
                            switch (c) {
                                ...
                                {   
                                    ...
                                    switch (c) {
                                        case '\\':
                                         {
                                            char d1 = parse_hex(getNext());
                                            char d2 = parse_hex(getNext());
                                            char str[2];
                                            str[0] = (d1 << 4) | d2;
                                            str[1] = 0;
        ...
        private:
            ...
            char parse_hex(const char value) const 
            {
            ...
            void parse_x() {
                ...
                while (1)
                {
                    char c = getNext();
                    if (c == '\\')  {
                        std::vector<char>::iterator iterOld = iter;
                ...
                std::u16string u16str(reinterpret_cast<char16_t*>(&bytes[0]), bytes.size() / T);
                std::wstring_convert<std::codecvt_utf8_utf16<char16_t>,char16_t> convert; 
                std::string utf8 = convert.to_bytes(u16str);
                std::copy(utf8.begin(), utf8.end(), std::back_inserter(result));
            }
