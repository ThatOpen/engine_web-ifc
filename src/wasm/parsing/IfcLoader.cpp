...
namespace webifc
{
  ...
  std::unordered_map<uint32_t, std::vector<uint32_t>> &IfcLoader::GetRelNests()
  {
    return _relNests;
  }
  ...
  void IfcLoader::PopulateRelNestsMap()
  {
    auto relNests = GetExpressIDsWithType(ifc::IFCRELNESTS);

    for (uint32_t relNestID : relNests)
    {
      uint32_t lineID = ExpressIDToLineID(relNestID);
      auto &line = GetLine(lineID);
      ...
      for (auto &nest : nests)
      {
        ...
        _relNests[relatingBuildingElement].push_back(nestID);
      }
    }
  }
  ...
  {
    auto relAggregates = GetExpressIDsWithType(ifc::IFCRELAGGREGATES);

    for (uint32_t relAggregateID : relAggregates)
    {
      uint32_t lineID = ExpressIDToLineID(relAggregateID);
      auto &line = GetLine(lineID);
