...
namespace webifc::geometry
{
  ...
  void IfcGeometryLoader::ComputeCurve(uint32_t expressID, IfcCurve &curve, uint8_t dimensions, bool edge, int sameSense, int trimSense, IfcTrimmingArguments trim) const
  {
      ...
      else if (dimensions == 3)
      {
        ...
        if (_loader.GetTokenType() != parsing::IfcTokenType::EMPTY)
        {
          ...
          for (auto &sg : pnSegment)
          {
            if (sg.type == "IFCLINEINDEX")
            {
              auto pts = ReadIfcCartesianPointList3D(pts2DRef);
              for (auto &pt : sg.indexs)
              {
                curve.Add(pts[pt - 1]);
              }
    ...
    case schema::IFCCIRCLE:
    {
      ...
      if (trim.exist)
      {
        ...
        if (trim.start.hasLenght && trim.end.hasLenght)
        {
          ...
          startDegrees = (startLenght / radius) * (180 / CONST_PI) - 90;
          endDegrees = (endLenght / radius) * (180 / CONST_PI) - 90;
        }
    ...
    case schema::IFCCURVESEGMENT:
    {
      ...
      trim.start.hasLenght = true;
      trim.end.hasLenght = true;
      IfcCurve newCurve;
      ComputeCurve(curveID, newCurve, 3, false, -1, -1, trim);

      glm::dmat3 placement = GetAxis2Placement2D(placementID);

      // TODO: review this part, not sure that reseting the starting point to zero is the proper way to do it
      for (size_t j = 1; j < newCurve.points.size(); j++)
      {
        newCurve.points[j] = glm::dvec3(newCurve.points[j].x - newCurve.points[0].x,
                                        newCurve.points[j].y - newCurve.points[0].y,
                                        newCurve.points[j].z - newCurve.points[0].z);
      }
      // Not sure of reseting the starting point
      newCurve.points[0] = glm::dvec3(0, 0, 0);

      // Transformation
      for (size_t j = 0; j < newCurve.points.size(); j++)
      {
        newCurve.points[j] = placement * glm::dvec3(newCurve.points[j].x, newCurve.points[j].y, 1);
        curve.Add(newCurve.points[j]);
      }
  ...
  glm::dmat4 IfcGeometryLoader::GetLocalPlacement(uint32_t expressID, glm::dvec3 vector) const
  {
    ...
    case schema::IFCPOINTBYDISTANCEEXPRESSION:
    {
      ...
      if (t == parsing::IfcTokenType::REF)
      {
        _loader.StepBack();
        auto curveId = _loader.GetRefArgument();
        curve = GetCurve(curveId, 3, false);
