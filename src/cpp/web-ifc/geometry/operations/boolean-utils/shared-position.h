    ...
    */
    struct Line
    {
        ...
        */
        ...
        //============================================================================================
        bool ContainsPos(double pos, double tol = tolerancePointOnLine) const
        {
            if (points.empty()) return false;
            for (size_t i = 1; i < points.size(); ++i) {
                double minv = points[i - 1].first;
                double maxv = points[i].first;
                if (pos >= minv - tol && pos <= maxv + tol) return true;
            }
            return false;
        }
    ...
    struct SharedPosition
    {
        ...
        bool ShouldFlip(Triangle& T, Triangle& neighbour)
        {
            ...
            */
            ...
            TriangleVsPoint AvsN = CalcTriPt(neighbour, A);
        ...
        //============================================================================================
        std::unordered_map<glm::ivec3, std::vector<size_t>> pointHashMap; // Maps quantized key to list of point IDs
        // Optimized AddPoint:
        size_t AddPoint(const Vec3& newPoint) {
            // Quantize the point based on tolerance
            double invTol = 1.0 / toleranceVectorEquality;
            glm::ivec3 primaryKey(
                static_cast<int>(std::floor(newPoint.x * invTol)),
                static_cast<int>(std::floor(newPoint.y * invTol)),
                static_cast<int>(std::floor(newPoint.z * invTol))
            );
            // Check primary and neighboring cells with radius 2
            const int RADIUS = 2;
            for (int dx = -RADIUS; dx <= RADIUS; ++dx) {
                for (int dy = -RADIUS; dy <= RADIUS; ++dy) {
                    for (int dz = -RADIUS; dz <= RADIUS; ++dz) {
                        glm::ivec3 neighKey = primaryKey + glm::ivec3(dx, dy, dz);
                        auto it = pointHashMap.find(neighKey);
                        if (it != pointHashMap.end()) {
                            for (size_t existingId : it->second) {
                                if (equals(points[existingId].location3D, newPoint, toleranceVectorEquality)) {
                                    return existingId; // Match found
                                }
                            }
                        }
                    }
            ...
            // No match: add new point
            Point p;
            p.id = points.size();
            p.location3D = newPoint;
            points.push_back(p);
            // Insert into primary cell's list
            pointHashMap[primaryKey].push_back(p.id);
            return p.id;
        }
        ...
        {
            std::vector<std::pair<double, size_t>> pointsInOrder = l.points;
            std::sort(
                pointsInOrder.begin(), pointsInOrder.end(), [&](const std::pair<double, size_t>& left, const std::pair<double, size_t>& right)
                { return left.first < right.first; });
            std::vector<std::pair<size_t, size_t>> segmentsWithoutIntersections;
            if (pointsInOrder.empty())
                return {};
            size_t cur = pointsInOrder[0].second;
            for (size_t i = 1; i < pointsInOrder.size(); i++)
            {
                ...
                size_t next = pointsInOrder[i].second;
                if (cur != next)
                {
            ...
            for (auto& line : p.lines)
            {
                ...
                for (auto& segment : segments)
                {
                    ...
                    if (projectedIndexA != projectedIndexB)
                    {
                        ...
                        edges.insert(std::make_pair(std::min(projectedIndexA, projectedIndexB), std::max(projectedIndexA, projectedIndexB)));
                    }
                }
            }
#ifdef CSG_DEBUG_OUTPUT
    ...
    inline std::vector<double> ComputeInitialIntersections(Plane& p, SharedPosition& sp, const Line& lineA)
    {
        ...
        for (auto& line : p.lines)
        {
            ...
            if (lineA.IsCollinear(line))
                continue;
            Vec3 relative = lineA.origin - line.origin;
            Vec3 cr = glm::cross(lineA.direction, line.direction);
            double denom = glm::dot(cr, p.normal);
            if (std::fabs(denom) < EPS_SMALL)
                continue;
            double t = -glm::dot(glm::cross(relative, line.direction), p.normal) / denom;
            Vec3 point = lineA.origin + t * lineA.direction;
            if (!line.IsPointOnLine(point))
                continue;
            double distB = line.GetPosOnLine(point);
            if (!line.ContainsPos(distB))
                continue;
            if (!p.IsPointOnPlane(point))
            {
                if (messages)
                {
            ...
            double distA = lineA.GetPosOnLine(point);
            distances.emplace_back(distA);
        }
        const auto double_less = +[](double left, double right)
            { return left < right; };
    ...
    {
        if (lineA.IsCollinear(lineB))
            return;
        Vec3 relative = lineA.origin - lineB.origin;
        Vec3 cr = glm::cross(lineA.direction, lineB.direction);
        double denom = glm::dot(cr, p.normal);
        if (std::fabs(denom) < EPS_SMALL)
            return;
        double t = -glm::dot(glm::cross(relative, lineB.direction), p.normal) / denom;
        Vec3 point = lineA.origin + t * lineA.direction;
        if (!lineA.IsPointOnLine(point) || !lineB.IsPointOnLine(point))
            return;
        double distA = lineA.GetPosOnLine(point);
        double distB = lineB.GetPosOnLine(point);
        if (!lineA.ContainsPos(distA) || !lineB.ContainsPos(distB))
            return;
        size_t point_id = sp.AddPoint(point);
        lineA.AddPointToLine(distA, point_id);
        lineB.AddPointToLine(distB, point_id);
        ReferenceLine refA;
        refA.pointID = point_id;
        refA.lineID = lineA.id;
        refA.location = distA;
        sp.points[point_id].lines.push_back(refA);
        ReferenceLine refB;
        refB.pointID = point_id;
        refB.lineID = lineB.id;
        refB.location = distB;
        sp.points[point_id].lines.push_back(refB);
        sp.AddRefPlaneToPoint(point_id, p.id);
    }
    //============================================================================================
