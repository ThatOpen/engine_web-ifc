...
var import_web_ifc = __toModule(require("web-ifc"));
var IfcThree = class {
  constructor(ifcAPI) {
    this.ifcAPI = ifcAPI;
  }
  LoadAllGeometry(scene, modelID) {
    ...
    const startUploadingTime = import_web_ifc.ms();
    ...
    console.log(`Uploading took ${import_web_ifc.ms() - startUploadingTime} ms`);
  }
  getFlatMeshes(modelID) {
    const startGeomTime = import_web_ifc.ms();
    const flatMeshes = this.ifcAPI.LoadAllGeometry(modelID);
    const endGeomTime = import_web_ifc.ms();
